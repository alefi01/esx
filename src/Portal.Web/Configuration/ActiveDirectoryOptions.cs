...
public sealed class ActiveDirectoryOptions
{
    ...
    /// <summary>
    /// С чего должны начинаться названия подразделений, показываемых
    /// на ПЕРВОМ уровне обзора каталога.
    ///
    /// В корне домена лежит всё подряд: служебные разделы, ветки чужих
    /// подразделений, остатки прошлых реорганизаций. Выдавать доступ
    /// оттуда некуда, а искать среди этого нужное — долго. Поэтому первый
    /// уровень можно сузить до своих веток, например «ВГ» и «УЦ».
    ///
    /// Пусто — показывать всё. Действует ТОЛЬКО на первом уровне: внутри
    /// ветки видно всё её содержимое, иначе отбор прятал бы подразделения
    /// с другими названиями глубже по дереву.
    /// </summary>
    public string[] RootBranchPrefixes { get; set; } = [];
}
