...
public static class OfficeDocuments
{
    ...
    private static string ExcelToHtml(ZipArchive archive)
    {
        ...
        // Книга помечается отдельно: у неё, в отличие от текстового документа,
        // ширина задаётся не страницей, а самой широкой таблицей. Полоса
        // прокрутки вбок должна быть одна на всё окно предпросмотра, внизу —
        // как вертикальная, а не своя у каждого листа (см. .doc--book в стилях).
        return sheets == 0
            ? Empty("В книге нет листов с данными.")
            : "<div class=\"doc doc--book\">" + html + "</div>";
    }
