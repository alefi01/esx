...
public sealed class FolderTree
{
    ...
    public FolderAccess AccessFor(ClaimsPrincipal user, StorageFolder folder)
    {
        ...
        // ПАПКА, КОТОРУЮ НИКТО НЕ НАСТРАИВАЛ, ВИДНА ВСЕМ.
        //
        ...
        // «выдайте всем чтение», и первое, чему учились, — выдавать всем
        // чтение сразу.
        //
        // Поэтому пока папке (и всей цепочке над ней) не назначено НИ ОДНОГО
        // права и наследование нигде не выключено, действует общее чтение:
        // видеть и скачивать может каждый, у кого есть доступ к порталу.
        // Как только кто-то выдал первое право или закрыл наследование,
        // папка становится обычной — действуют только заданные права.
        if (!configured && result < FolderAccess.Read)
        {
