...
public sealed class UploadValidator
{
    ...
    /// <summary>
    /// Запрещённые расширения — те же, по которым проверяет Validate.
    ///
    /// Нужны странице файлов: браузер отсеивает заведомо негодный файл
    /// ещё до отправки, не гоняя его по каналу впустую. Это УДОБСТВО,
    /// а не защита — настоящая проверка всё равно делается здесь, на сервере,
    /// и отключённый в браузере код её не обходит.
    /// </summary>
    public IReadOnlyCollection<string> BlockedExtensions => _blocked;
