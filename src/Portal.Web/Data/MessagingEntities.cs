...
public class Message : ISyncable
{
    ...
    /// <summary>
    /// Адрес, с которого сообщение отправлено.
    ///
    /// Показывается ТОЛЬКО администратору портала и только когда он смотрит
    /// чужую переписку — рядом с именем автора. Участникам беседы адреса
    /// друг друга не показываются: им это ничего не даёт, а вопросов
    /// вызывает много.
    ///
    /// Зачем вообще: разбор случаев вроде «с моей учётки писали не я».
    /// Тот же приём, что и в журнале действий с файлами, где адрес
    /// пишется с самого начала.
    ///
    /// Пусто — сообщение отправлено до появления этой записи.
    /// </summary>
    [MaxLength(64)]
