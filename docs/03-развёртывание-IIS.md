...
> **ВАЖНО, прочитайте прежде чем править настройки.**
>
> Команда `dotnet publish` **перезаписывает `appsettings.json`** содержимым
> из репозитория. Всё, что вы поправите в `C:\inetpub\ftp\appsettings.json`,
> пропадёт при следующем обновлении портала — и, скорее всего, вы обнаружите
> это не сразу, а по неработающему входу.
>
> Поэтому все настройки, специфичные для вашего сервера, задавайте
> в отдельном файле **`appsettings.Production.json`**. Он перекрывает
> `appsettings.json`, в репозиторий не попадает и публикацией не затирается.

Создайте `C:\inetpub\ftp\appsettings.Production.json` и укажите в нём
...
Если подсети офисов отличаются от заданных по умолчанию, добавьте
и секцию `Offices`. Учтите: массивы в конфигурации **не сливаются**,
а заменяются целиком, — перечисляйте все офисы сразу.

```json
{
  "Offices": {
    "Items": [
      { "Code": "office1", "Name": "Офис 1", "Subnets": [ "192.168.96.0/20" ],  "DomainControllers": [ "192.168.96.3"  ] },
      { "Code": "office2", "Name": "Офис 2", "Subnets": [ "192.168.112.0/20" ], "DomainControllers": [ "192.168.112.2" ] }
    ]
  }
}
```
...
**Как убедиться, что настройки применились.** Откройте на самом сервере
`http://localhost/Admin/LoginTest` — там показаны фактические значения,
с которыми работает приложение, а не то, что написано в файле.
Если они не совпадают с ожидаемыми, перезапустите пул приложений.
...
`App_Data` и `appsettings.Production.json` при этом не затираются:
ключи сохранятся, пользователей не выкинет из системы, настройки сервера
останутся на месте.

А вот `appsettings.json` перезаписывается всегда. Если вы правили его
напрямую — правки пропадут. Все свои настройки держите
в `appsettings.Production.json` (см. шаг 4).
...
### После обновления перестал работать вход у всех пользователей

Первым делом откройте **на самом сервере**:

```
http://localhost/Admin/LoginTest
```

Эта страница доступна с консоли сервера без входа в портал (снаружи она
отвечает «404»). Она показывает фактические настройки подключения к домену
и, если ввести логин с паролем, — на каком именно шаге всё останавливается.

Самая частая причина — как раз затёртый публикацией `appsettings.json`
(см. предупреждение в шаге 4). Признак: на странице проверки видно
не то значение `BaseDn` или `DomainFqdn`, которое вы задавали.
